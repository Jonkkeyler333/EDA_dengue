...
```{r}
dbscan::kNNdistplot(pca$ind$coord[,1:30],k=25)
clusters<-dbscan(pca$ind$coord[,1:30],eps=7.6,minPts=25)
print(length(clusters$cluster))
...
```
Se empleó una metrica para evaluar la calidad de agrupamiento que realizó el algoritmo 
```{r}
silhouette_result1<-silhouette(clusters$cluster,dist(pca$ind$coord[,1:30]))
summary(silhouette_result1[,3])
```
```{r}
dbscan::kNNdistplot(df_ns,k=10)

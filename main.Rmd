...
Dado que no se reportaron fallecimientos entre los casos de dengue grave, el estudio se centró exclusivamente en los 26 casos que sobrevivieron. Se exploró perfil sociodemográfico y clínico de estos pacientes.
Para ello se analizó la distribución de los casos para diferentes variables socioeconomicas, demograficas y clinicas con el fin de hallar algun tipo de patrón.
...
---(no se si podria entrar en analisis de correspondencia:)---
Finalmente, se analizó la relación entre los síntomas mediante una matriz de correlación. Este análisis permitió identificar grupos de síntomas que podrían estar relacionados entre sí.
...
---POSIBLE MODIFICACION, ESA LIBRERIA DEJA HACER TABLAS MAS DINAMICAS PARA CUANDO TOQUE PRESENTAR UN LISTADO DE DATOS MAS O MENOS EXTENSO---
```{r}
datatable(contribs[orden,], 
          options = list(pageLength = 10,  # Esto es para las  filas por página
                         lengthMenu = c(10, 25, 50, 100),  # Esto es para las opciones que salen en "show entries"
                         searchHighlight = TRUE),  
          caption = 'Conjunto de datos final a utilizar')
```
...
--MODIFICACION--
Para los casos concretos de dengue grave se obtuvo lo siguiente:
Para los casos de dengue grave, también se observa en la distribución por sexo una mayor frecuencia de casos en hombres (M) en comparación con las mujeres (F). Y que la mayoría se concentrarón en adultos de 29 años o más.
```{r}
# Visualización: gráfico de barras para el sexo
ggplot(dengue_grave, aes(x = sexo_)) +
  geom_bar(fill = "tomato") +
  theme_minimal() +
  labs(title = "Distribución por sexo",
       x = "Sexo", y = "Frecuencia")

# Visualización: histograma para la edad
ggplot(dengue_grave, aes(x = clasif_edad)) +
  geom_bar(fill = "steelblue") +
  theme_minimal() +
  labs(title = "Distribución por clasificación de edad",
       x = "Clasificación de edad", y = "Frecuencia")
```
...
En cuanto a la correlación que presentan los sintomas entre sí, se puede apreciar que los que tienen mayor correlación (>0.8) son _fiebre_ y _malgias_ (0.85), _fiebre_ y _cefalea_ (0.83). Por otro lado, los sintomas mas graves como hipotensión, extravasación de líquidos, choque y daño orgánico casi no presentan correlación con los demas sintomas.
(en el analsis voy explicar q tambien es por la cantidad de datos de cada tipo de dengue, como solo son 26 graves, casi no hay sintomas como hemorragias y así en general, por eso sale valores tan pequeños)
...
=======
>>>>>>> f40e38467a79f3166063958e5cae100137f461d0
